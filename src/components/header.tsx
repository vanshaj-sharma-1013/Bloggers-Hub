      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <div className={`overflow-hidden transition-all duration-300 ${isScrolled ? "w-0 opacity-0" : "w-auto opacity-100"}`}>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {[
              ...
              <Link
                key={item.label}
                href="#"
                className={`font-medium text-slate-700 dark:text-slate-300 dark:${item.color} hover:text-slate-900 dark:hover:text-slate-100 transition-colors duration-200 px-3 py-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/50 ${
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <button
              ...
              aria-label="Search"
            >
              <Search className={`text-slate-600 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors ${isScrolled ? "w-4 h-4" : "w-5 h-5"}`} />
            </button>
